  <script>
    ...
    async function startQuiz() {
      try {
        ...
        if(state.questions.length===0) throw new Error("Banco de vídeos vazio.");
    ...
    function loadQuestion() {
      ...
      document.getElementById('progress-text').innerText = `Vídeo ${state.currentIdx + 1}/${state.questions.length}`;
      ...
      if (q.modo === 'padrao') {
          ...
          html += `<label class="radio-option"><input type="radio" name="choice" value="video_a" required> Vídeo da Esquerda (A) é IA</label>`;
          html += `<label class="radio-option"><input type="radio" name="choice" value="video_b"> Vídeo da Direita (B) é IA</label>`;
